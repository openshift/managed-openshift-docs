...
--enable-autoscaling
|Enable autoscaling of compute nodes. Use this argument with the `--min-replicas` and `--max-replicas` arguments. To disable autoscaling, use `--enable-autoscaling=false` with the `--replicas` argument.
...
Reshape the autoscaling range on a machine pool named `mp1` on a cluster named `mycluster`.
