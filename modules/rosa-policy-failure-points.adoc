...
[role="_abstract"]
{product-title} provides many features and options for protecting your workloads against downtime, but applications must be architected appropriately to take advantage of these features.
...
== Cluster failure
{product-title} clusters have at least three master nodes and three infrastructure nodes that are preconfigured for high availability, either in a single zone or across multiple zones, depending on the type of cluster you have selected. Master and infrastructure nodes have the same resiliency as worker nodes, with the added benefit of being managed completely by Red Hat.

In the event of a complete master outage, the OpenShift APIs will not function, and existing worker node pods are unaffected. However, if there is also a pod or node outage at the same time, the masters must recover before new pods or nodes can be added or scheduled.
