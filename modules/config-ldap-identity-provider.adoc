...
.Procedure
To configure a LDAP identity provider for your {product-title} cluster:
...
. Enter a *Bind DN* and *Bind password*.

. Enter an *ID* attribute whose value should be used as the user ID. Click *Add more* to add multiple ID attributes.

. Enter a *Preferred username* attribute whose value should be used as the display name. Click *Add more* to add multiple preferred username attributes.
